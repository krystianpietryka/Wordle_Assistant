...
def Intro():
    layout = [
        [sg.Text("Welcome to Wordle Assistant!", size=(40, 1))],
        [
            sg.Text("Green Letters:      "),
            sg.InputText("", size=(2, 1), key="green1", enable_events=True),
        ...
        [
            sg.Text("Yellow Letters:     "),
            sg.InputText("", size=(2, 1), key="yellow1", enable_events=True),
        ...
        [
            sg.Text("Excluded Letters: "),
            sg.InputText("", size=(18, 1), key="excluded", enable_events=True),
        ],
...
def Help():
    layout = [
        [
            sg.Text(
                """ Hi! Thank you for using Wordle Assistant!
...
def Best_Starters():
    layout = [
        [
            sg.Text(
                """The best starting guesses (according to a few internet pages) in no particular order are as follows:
    *STARE
    *IRATE
    *SOARE
    *ROATE
...
def Main():
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED or event == "Exit":
            ...
            green = Wordle_Answers_For_Input.Convert_Empty_Letters(
                (
                    values["green1"],
                    values["green2"],
                    values["green3"],
            ...
            yellow = Wordle_Answers_For_Input.Convert_Empty_Letters(
                (
                    values["yellow1"],
                    values["yellow2"],
                    values["yellow3"],
            ...
            valid_symbol_flag = Wordle_Answers_For_Input.Valid_Symbol_Check(
                green + yellow
            )

            # Same Place Letter Check for green and yellow letters input
            same_place_flag = Wordle_Answers_For_Input.Same_Place_Letters_Check(
                green, yellow
            )
            ...
            if valid_symbol_flag == 1:
                if same_place_flag == 1:
                    result = Wordle_Answers_For_Input.Display_Possible_Answers(
                        possible_answers,
                        values["excluded"].lower(),
                        green.lower(),
                        yellow.lower(),
                    )
                    ...
                    window2["-OUTPUT-"].update(
                        "Yellow and Green letters exist at the same index!"
                    )
            else:
                window2 = Answers(10)
